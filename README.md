...
## Daten
Die Daten/Menüs sind über den Link *<span>https://</span>mensa.v2.software-engineering.education/**Stadt**/**Gastronomie**/**Tag*** verfügbar. Dabei müssen **Stadt** und **Gastronomie** durch einen Code für einen Ort und eine Mensa (siehe Places enum) und **Tag** durch einen englischen Wochentag (siehe Weekday enum) ausgetauscht werden. Die API liefert als Antwort einen JSON-formatierten String. Die oberste Ebene bildet dabei ein JSON-Objekt, in diesem ist ein JSON Array unter dem Bezeichner *data* enthalten, in welchem mehrere JSON-Objekte enthalten sind, die jeweils ein Gericht repräsentiert. Die Objekte haben die folgende Form:

```json
{
    "description":"List of dishes at canteen [Mensa] at location [Regensburg] for day [Wednesday]",
    "root":"https://mensa.v2.software-engineering.education",
    "createdAt":"2022-06-06T10:19:30.174Z",
    "data": [
        {
            "day":"Wednesday",
            "date":"09.06.2021",
            "name":"Indisches Gem�securry ",
            "category":"HG1",
            "labels":"VG",
            "prices": {
                "forStudents":"2,00",
...
```
Für uns relevant sind nur die Objekte im *data* Array und von diesem nur die Felder `name`, `day` und `category`. Sie können die API auch ohne App testen, rufen Sie dazu einfach die URL im Browser auf. (z.B. https://mensa.v2.software-engineering.education/Regensburg/Mensa/Wednesday oder https://mensa.v2.software-engineering.education/ zum durckklicken).
...
Verwenden Sie nun die `JSONObject` Objekte, die Sie von der API erhalten, um basierend darauf Objekte der Klasse `MensaDish` zu erstellen. Ein Objekt dieser Klasse repräsentiert dabei genau ein Gericht. Schreiben Sie dazu eine neue Methode (bestenfalls statisch in der `MensaDish` Klasse), die ein `JSONObject` Objekt übergeben bekommt und ein fertiges `MensaDish` Objekt zurückgibt. Nutzen Sie diese, um alle Gerichte der aktuellen Woche als MensaDishes abzuspeichern und fügen Sie diese zu Ihrer Liste an aktuellen Gerichten hinzu.
